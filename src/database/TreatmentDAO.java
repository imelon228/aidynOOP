...
public class TreatmentDAO {
    ...
    public boolean insertVaccination(Vaccination vaccination) {
        String sql = "INSERT INTO treatment (treatment_name, cost, is_completed, vaccine_name, dose_number) " +
                "VALUES (?, ?, ?, ?, ?)";
    ...
    public boolean insertSurgery(Surgery surgery){
        String sql = "INSERT INTO treatment (treatment_name, cost, is_completed, anesthesia_type, risk_level) " +
                "VALUES (?, ?, ?, ?, ?)";
    ...
    public boolean updateVaccination(Vaccination vaccination) {
        String sql = "UPDATE treatment SET cost = ?, is_completed = ?, vaccine_name = ?, dose_number=? " +
        ...
        try {
            ...
            statement.setBoolean(2, vaccination.getComplete());
            statement.setString(3, vaccination.getVaccineName());
            statement.setInt(4, vaccination.getDoseNumber());
            statement.setInt(5, vaccination.getTreatmentID());
    ...
    public boolean updateSurgery(Surgery surgery){
        String sql = "UPDATE treatment SET cost = ?, is_completed = ?, anesthesia_type = ?, risk_level = ?"
        ...
        try{
            ...
            statement.setBoolean(2, surgery.getComplete());
            statement.setString(3, surgery.getAnesthesiaType());
            statement.setInt(4, surgery.getRiskLevel());
            statement.setInt(5, surgery.getTreatmentID());
    ...
    public void demonstratePolymorphism() {
        ...
        if (treatmentList.isEmpty()) {
            ...
            for (Treatment s : treatmentList) {
                System.out.println(s.getTreatmentID() + "   " + s.calculateCost());
            }
        }
    ...
    private Treatment extractTreatmentFromResultSet(ResultSet resultSet) throws SQLException {
        ...
        if ("Vaccination".equals(treatmentName)) {
            ...
            treatment = new Vaccination(treatmentId, cost, completed, vaccineName, doseNumber);
            ...
            treatment = new Surgery(treatmentId, cost,completed, anesthesiaType, riskLevel);
        }
